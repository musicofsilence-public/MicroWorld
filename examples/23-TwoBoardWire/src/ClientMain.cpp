...
void RunClient() noexcept
{
	...
	static FWireBinding Wire{Net, AppWireChannelByte, AppChannelId, EChannelSendTarget::Server, Router};
	...
	FNetHostConfig ClientConfig = MakeHostConfig();
	ClientConfig.ServerAddress = MakeUartAddress(ServerNodeId);
	(void)Net.Configure(ENetMode::Client, ClientConfig);
