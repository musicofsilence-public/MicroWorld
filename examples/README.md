...
| 16 | `16-TwoBoardUdp` | Messaging + Networking client/server over WiFi (SoftAP, no router) | 2nd board | ✅ hardware-verified 2026-08-05 |
| 17 | `17-TwoBoardLora` | RadioE32 through `FEsp32LoraDevice`; direct loop advances queued TX | 2nd board, 2 × E32 | ✅ |
| 18 | `18-TwoBoardUart` | wired UART transport-device link — example 17's volley over a plain wire | 2nd board, 3 wires | 🟨 |
| 19 | `19-UartMessaging` | Messaging + Networking client/server design over UART, zero WiFi — example 16's protocol, only the device changed | 2nd board, 3 wires | 🟨 built; Network path not hardware-verified |
| 20 | `20-TwoBoardI2c` | wired I2C master/slave transport-device link — example 18's volley over a clocked bus | 2nd board, 3 wires + 2 pull-ups | 🟨 |
| 21 | `21-TwoBoardSpi` | wired SPI master/slave transport-device link — example 20's volley over a clocked full-duplex bus | 2nd board, 5 wires | 🟨 |
| 22 | `22-ActorMessages` | local actor messaging: one `FMessagingSystem` channel with no device, broadcast + filtered subscription, one board | — | 🟨 |
| 23 | `23-TwoBoardWire` | actor messaging over a UART wire: one channel naming a UART device, switch drives lamp | 2nd board, 3 wires | 🟨 |
| 24 | `24-TwoChannelWorld` | two channels, one world: telemetry over WiFi UDP + commands over a UART wire on one `FMessagingSystem` | 2nd board, 3 wires + WiFi | ✅ |
| 25 | `25-GuaranteedDelivery` | best-effort vs guaranteed delivery on one WiFi-UDP link: a `bIsReliable` channel recovers packets `FPacketDropDevice` drops | 2nd board + WiFi | ✅ |
| 26 | `26-MessagingOverLora` | Messaging + Networking client/server design over E32 LoRa — example 19's protocol at the D8 airtime profile (heartbeat 3 s, state broadcast paced 1 s) | 2nd board, 2 × E32 | 🟨 built; migrated Network path not hardware-verified |

Wired board-to-board transports (examples 18–21) share one pattern: swapping the
device line swaps the transport, and everything above it stays put.
