...
protected:
	 ...
	 * Responsibilities: Increment its supplied counter once.
	 */
...
/**
 * Motivation: Accepts one valid remote spawn request without exposing a transport route.
...
void HandleSpawnRequest(const FMessage& InMessage) noexcept
{
	...
	++GSpawnSequence;
	++GWorldActorCount;
	MW_LOG(Log, "ex16", "server spawned actor -> world actor count=%d", GWorldActorCount);
}

/**
 * Motivation: Builds the server composition after the demonstration access point is ready.
 * Responsibilities: Register the device, create the Engine-owned systems, and install the bounded spawn handler.
 */
bool SetupServer(FEsp32WifiDevice& Device, TPlaySystemSet<1>& DeviceFrames, FServerEngine& ServerHost, FNetworkSystem*& OutNetwork) noexcept
{
	if (DeviceFrames.Add(Device) != EEngineResult::Success || ServerHost.CreateMessagingSystem({}) != ERuntimeResult::Success)
	{
		MW_LOG(Error, "ex16", "server messaging setup failed; halting");
		return false;
	}
	...
	{
		MW_LOG(Error, "ex16", "server network setup failed; halting");
		return false;
	}
...
void RunServerMessageLoop(FServerEngine& ServerHost, FNetworkSystem& Network) noexcept
{
	...
	for (;;)
	{
		...
		{
			MW_LOG(Log, "ex16", "done (server spawned %d actors)", GWorldActorCount);
			bDoneAnnounced = true;
		}
...
/**
 * Motivation: Runs the example's server role with device ownership below Messaging and Network.
 * Responsibilities: Host the AP, admit clients, and broadcast the bounded world state.
 */
void RunServer() noexcept
{
	static FEsp32WifiLink WifiLink;
	if (WifiLink.StartAccessPoint(FEsp32AccessPointConfig{DemoApSsid, DemoApPassword, 1, 4}) != ETransportResult::Success)
	{
