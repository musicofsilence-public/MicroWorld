...
`FEsp32I2cMasterDevice` and `RunMaster`, the slave build an
`FEsp32I2cSlaveDevice` and `RunSlave`, so each environment compiles only its role.
...
- The volley loop above the device is otherwise example 18's, so this is the
  same ping-pong with only the device construction and the who-clocks-the-bus
  pacing changed.
...
- Static device and RX buffer, never `app_main` stack locals (§2.2). No WiFi and
