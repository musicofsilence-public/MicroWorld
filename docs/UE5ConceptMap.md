...
| Network byte I/O | `IDevice`, `TTransportManager`, `FByteWriter`/`FByteReader`, `FDeviceAddress`, `THostLoopback` | Transport (0.4.0) | One non-blocking addressed device, fixed-capacity caller-storage-backed manager, bounded bytes over caller-owned spans, transactional failure semantics, deterministic host loopback independent of Engine |
