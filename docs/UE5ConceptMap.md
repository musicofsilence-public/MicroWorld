...
| Platform adapters (time / UDP / LoRa) | `FEsp32TimeSource`, `FHostUdpDriver`, `FEsp32UdpDriver`, `FEsp32E32LoraDriver`, `WriteEsp32LogRecord` | platform-host / platform-esp32 (0.2.0, non-portable) | Supply the real transports and clock behind `INetDriver` / `TimePointMilliseconds` / `FOutputDeviceFunction`; depend inward on portable packages, never the reverse |
