...
| Managed World / Actor / Component | `UWorld`, `AActor`, `UActorComponent`, `FObjectInitializer::CreateDefaultSubobject` | Engine (0.6.0) | Application roots World; World/Actor trace children; an actor may create its own default components only during its private construction transaction; parent references are weak |
...
| Application entry point / game instance | `TEngine<TTraits>::ConfigureNetworking` behind `IEngineRuntime` | Engine (0.6.0) | Configures one Engine-owned Network from a borrowed device and Engine-owned setup value before World creation; owns registry/store/GC/world/timers and lifecycle order bound devices → Messaging → Network → World on begin/pre, then World → Network → Messaging → bound devices on post/end |
...
| Networking with roles | `FNetworkSystem`, `ENetworkRole`, `FPeerId` | Networking (0.6.0) | Client or Server only; an optional initial server route is owned by Network and starts on its first frame; configured server admission may be lower than fixed four-peer storage. Heartbeat/timeout, addressed send, and server broadcast use Messaging; the initial device address is setup data and routes remain internal afterward; no authentication or replication |
