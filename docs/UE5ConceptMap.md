...
| Platform adapters (time / UDP / LoRa) | `FEsp32TimeSource`, `FHostUdpDriver`, `FEsp32UdpDriver`, `FEsp32E32LoraDriver`, `Esp32OutputDevice` | platform-host / platform-esp32 (0.2.0, non-portable) | Supply the real transports and clock behind `INetDriver` / `TimePointMilliseconds` / `FOutputDevice`; depend inward on portable packages, never the reverse |
