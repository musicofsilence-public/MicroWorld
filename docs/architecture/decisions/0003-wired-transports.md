...
- **Wired board-to-board links are network transports.** Each one is a new
  `INetDriver` implementation inside `Modules/MicroWorld/Platform/Esp32`, exactly like
...
- The portable packages (`Core`, `Memory`, `Object`, `Engine`, `Net`) and
  `PlatformHost` stay untouched; every new line lives in `Modules/MicroWorld/Platform/Esp32`
