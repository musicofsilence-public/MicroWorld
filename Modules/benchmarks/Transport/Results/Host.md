...
& (Join-Path $NetBuild 'Release\microworld_transport_tests.exe')
```
...
The consumer links `MicroWorld::Transport` (which pulls Core and Memory), compiles
...
& (Join-Path $ConsumerBuild 'Release\microworld_transport_consumer.exe')
```
...
python lib/microworld/tools/CheckProfileMap.py `
    --map $ConsumerBuild\microworld_transport_profile.map `
    --profile Core+Net `
