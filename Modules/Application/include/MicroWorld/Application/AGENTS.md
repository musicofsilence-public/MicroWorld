...
order: `Application.h` binds one `IEngine&`, seals the begin/tick/end forwarding
behind private non-virtual methods, leaves `OnConfigure` as the only hook that
can reach the world, and owns the `Run` member template that composes a clock,
a sleep function, and a frame period into the begin-then-advance-then-sleep loop.
...
- `FApplication::Run` is a member template so a platform names its concrete
  clock; it receives the clock by reference (it must outlive the call) and the
  sleep function pointer and frame period by value.
