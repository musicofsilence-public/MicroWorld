...
/** Motivation: Capacities shared by every router test; each test constructs its own fresh router instance. */
...
/** Motivation: Message type and actor ids shared across cases; values are arbitrary and only need to be distinct. */
...
/** Motivation: Distinct single-byte payload values each delivery and flush test threads through the router. */
...
/** Motivation: Single-byte payload count shared by every one-byte payload in this suite. */
constexpr std::size_t OneBytePayloadCount = 1;

/** Motivation: A valid-looking handle value used to prove failed registrations clear their output. */
constexpr FMessageHandlerHandle CanaryHandle{0u, 1u};

/**
 * Motivation: Records each delivered message's caller-supplied identity and header fields in invocation order.
...
{
	/** Motivation: Bounds the recorded sequence so test fixtures stay allocation-free and fixed-size. */
	static constexpr std::size_t MaxEntries = 8;

	/** Motivation: Tracks the next write position so later reads observe invocation-order dispatch. */
	std::size_t Count{0};

	/** Motivation: Stores the caller-supplied identity of each invoked handler. */
	int Identities[MaxEntries]{0};

	/** Motivation: Stores the TargetActorId each invocation observed. */
	FMessageActorId TargetActorIds[MaxEntries]{0};

	/** Motivation: Stores the SenderActorId each invocation observed. */
	FMessageActorId SenderActorIds[MaxEntries]{0};

	/**
	 * Motivation: Records one observed invocation and the view it was invoked with.
...
/**
 * Motivation: Binds a nothrow inline handler that records its identity and the delivered view in the shared
 *   recorder.
 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
 */
...
/**
 * Motivation: A minimal IMessageChannel test double that returns a scripted result sequence and records what it
 *   received.
 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
 * Example:
 *   // Construct and exercise the type in one behavior test.
...
{
public:
	/** Motivation: Bounds the scripted-result queue and the receive log without allocating. */
	static constexpr std::size_t MaxScriptedResults = 4;

	/** Motivation: Bounds how many accepted sends this stub retains for inspection. */
	static constexpr std::size_t MaxReceivedMessages = 4;

	/**
	 * Motivation: Binds this stub to one channel id and one MaxEncodedMessageBytes budget.
	 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
	 */
	...
	/**
	 * Motivation: Reports this stub's fixed channel id.
	...
	/**
	 * Motivation: Reports this stub's fixed per-message byte budget.
	 * Responsibilities: Return the stored value and touch nothing else.
	 */
	std::size_t MaxEncodedMessageBytes() const noexcept override { return MaxBytes; }

	/**
	 * Motivation: Appends one scripted result that a future call will return, in call order.
	 * Responsibilities: Perform only the documented mutation and leave unrelated state untouched.
	 */
	...
	/**
	 * Motivation: Returns the next scripted result (Success once the script is exhausted) and records an accepted
	 *   send's bytes.
	 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
	 */
	...
	/**
	 * Motivation: Reports how many TrySendEncodedMessage calls this stub has observed, succeeded or not.
	 * Responsibilities: Return the stored value and touch nothing else.
	 */
	std::size_t SendAttempts() const noexcept { return SendAttemptCount; }

	/**
	 * Motivation: Reports how many sends this stub accepted (returned Success).
	 * Responsibilities: Return the stored value and touch nothing else.
	 */
	std::size_t ReceivedCount() const noexcept { return ReceivedMessageCount; }

	/**
	 * Motivation: Reports the encoded length of the Index'th accepted send.
	 * Responsibilities: Return the stored value and touch nothing else.
	 */
	std::size_t ReceivedLength(const std::size_t InIndex) const noexcept { return ReceivedLengths[InIndex]; }

	/**
	 * Motivation: Reports one byte of the Index'th accepted send's encoded bytes.
	 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
	 */
...
private:
	/** Motivation: Bounds the per-message receive log to the router's own byte budget used across these tests. */
	static constexpr std::size_t MaxMessageBytes = MessageByteCapacity;

	/** Motivation: This stub's fixed channel id, returned by GetChannelId. */
	FMessageChannelId ChannelId;

	/** Motivation: This stub's fixed per-message byte budget, returned by MaxEncodedMessageBytes. */
	std::size_t MaxBytes;

	/** Motivation: Results returned in order by successive TrySendEncodedMessage calls. */
	EMessageResult ScriptedResults[MaxScriptedResults]{};

	/** Motivation: Number of results appended by ScriptResult so far. */
	std::size_t ScriptedResultCount{0};

	/** Motivation: Number of scripted results already consumed by TrySendEncodedMessage. */
	std::size_t ScriptedReadIndex{0};

	/** Motivation: Total TrySendEncodedMessage calls observed, regardless of the returned result. */
	std::size_t SendAttemptCount{0};

	/** Motivation: Total accepted sends recorded in ReceivedLengths and ReceivedBytes. */
	std::size_t ReceivedMessageCount{0};

	/** Motivation: Encoded length of each accepted send, indexed by acceptance order. */
	std::size_t ReceivedLengths[MaxReceivedMessages]{};

	/** Motivation: Encoded bytes of each accepted send, indexed by acceptance order. */
	std::uint8_t ReceivedBytes[MaxReceivedMessages][MessageByteCapacity]{};
};

/**
 * Motivation: Encodes one zero-payload message of MessageTypeId Type into a caller-owned fixed buffer for direct
 *   ReceiveEncodedMessage tests.
 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
 */
...
/**
 * Motivation: Register three handlers for the same type and broadcast a one-byte payload, then flush one frame.
 * Responsibilities: The broadcast enqueues successfully and reaches all three subscribers in registration order.
 */
...
/**
 * Motivation: Register same-type handlers for two distinct actor ids and send a message targeted at one, then
 *   flush one frame.
 * Responsibilities: The send enqueues successfully and reaches only the matching listener, carrying the original
 *   TargetActorId.
 */
...
/**
 * Motivation: Register a handler, broadcast a message, then drive separate PreAdvance and PostAdvance steps.
 * Responsibilities: The send enqueues but never invokes a handler inline; the message reaches inbound only on
 *   PostAdvance and delivers only on the next.
 */
...
/**
 * Motivation: Register an echoing handler that broadcasts a second type, plus an observer, then drive the
 *   triggering broadcast across three frames.
 * Responsibilities: The triggering broadcast enqueues successfully; the echoed message is not visible within the same
 *   dispatch pass and arrives exactly one.
 */
...
/**
 * Motivation: Bundles one in-dispatch self-mutation attempt so its captures fit the handler's inline delegate
 *   budget.
 * Responsibilities: Honour the contract in Motivation and own no behaviour beyond it.
 * Example:
 *   // Construct and exercise the type in one behavior test.
...
{
	/** Motivation: Handle of the handler performing the attempted mutation, set after its own registration. */
	FMessageHandlerHandle SelfHandle{};

	/** Motivation: Output handle from the attempted in-dispatch AddMessageHandler call. */
	FMessageHandlerHandle RejectedAddHandle{CanaryHandle};

	/** Motivation: Result of the attempted in-dispatch AddMessageHandler call. */
	EMessageResult AddResult{EMessageResult::Success};

	/** Motivation: Result of the attempted in-dispatch RemoveMessageHandler call (removing SelfHandle). */
	EMessageResult RemoveResult{EMessageResult::Success};

	/** Motivation: Records whether the attempt actually executed. */
	bool bObserved{false};
};

/**
 * Motivation: Register a self-mutating handler that attempts an add and a remove from inside dispatch, then
 *   trigger and flush it.
 * Responsibilities: The in-dispatch add and remove both return DispatchLocked; the rejected add clears its output handle
...
/**
 * Motivation: Register a handler, remove it once successfully, then remove the same handle a second time.
 * Responsibilities: The repeated removal returns StaleHandle.
 */
...
/**
 * Motivation: Fill every handler slot, then attempt one more registration with a bound delegate and a canary
 *   output handle.
 * Responsibilities: The fourth registration returns CapacityExceeded; the output handle is cleared, the input delegate
 *   stays bound, and occupancy is.
 */
...
/**
 * Motivation: Fill the outbound queue with two sends, then attempt one more broadcast.
 * Responsibilities: The third send returns CapacityExceeded and outbound occupancy stays unchanged.
 */
...
/**
 * Motivation: Fill the inbound queue with two receives, then receive one more encoded message.
 * Responsibilities: The third receive returns CapacityExceeded, increments DroppedInboundCount, and leaves inbound
 *   occupancy unchanged.
 */
...
/**
 * Motivation: Attempt to register a channel under LocalChannelId, then register a fresh id and attempt to register
 *   a duplicate of it.
 * Responsibilities: The LocalChannelId registration returns InvalidChannel; the duplicate registration returns
 *   Duplicate.
 */
...
/**
 * Motivation: Register a stub channel scripted to fail once, queue two sends, then flush twice.
 * Responsibilities: The failed flush attempts only the head entry and retains both queued entries; the next flush drains
 *   both once the channel accepts them,.
 */
...
/**
 * Motivation: Warm up the router once, then perform a steady-state cycle of send, flush, dispatch, removal, and
 *   re-registration.
 * Responsibilities: The steady-state cycle performs no observable allocation.
 */
