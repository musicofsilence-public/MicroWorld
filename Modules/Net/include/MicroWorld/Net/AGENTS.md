...
- `TNetPacketStorage<MaxPackets, MaxPacketBytes>` is the smallest fixed
  ...
  observed only by the matching `TNetManager` specialization.
- `TNetManager` copies complete accepted packets into the caller-supplied
  ...
  inbound queue.
- `THostLoopback` is a deterministic fixed-capacity loopback for host tests;
