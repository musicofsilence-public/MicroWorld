...
platform interfaces that separate SDK-free UART policy from the Pico SDK backend.
None contains Pico SDK access.

## Concepts

- Fixed framing and the legacy E32 platform spelling remain compatibility paths;
  RadioE32 owns framing while the generic `IPicoUartPlatform` interface owns byte access.
