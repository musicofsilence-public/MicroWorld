...
released Engine public API. They demonstrate consumer ownership and
`TEngineHost` lifecycle orchestration without becoming reusable production
modules; MicroWorld never depends on them.

## Concepts

- Concrete objects live in dependency-safe declaration order.
- Registration happens before `BeginPlay` and remains non-owning.
- Caller-provided timestamps keep output deterministic and testable.
- Example hooks expose Component/Actor tick independence without product,
