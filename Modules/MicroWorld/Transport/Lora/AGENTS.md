...
Portability is what makes that possible. The device borrows Core's
...
These sources are optional. `MICROWORLD_TRANSPORT_LORA` (declared in
...
`Internal/` holds the transport state header the device composes; consumers depend
on the public device and address headers only.
...
- A LoRa address is one byte of node id. Transparent-mode transmission is a
  broadcast, so the destination is device-relative metadata rather than an
...
Build the Transport target once with `MICROWORLD_TRANSPORT_LORA=OFF` after
