...
This directory holds one header and no device. `UdpAddressCodec.h` defines the
6-byte UDP address encoding — four IPv4 octets plus a port — that every UDP
adapter reads and writes.

The devices themselves are not portable and never will be: `Platform/Host`
...
- `IsUdpAddress` tests the active length only, so a one-byte LoRa or loopback
  ...
  when a device actually routes them.
- Byte-order conversion stays with the caller. A device applies `ntohl`/`ntohs`
...
Behaviour is covered indirectly by the host UDP device and end-to-end tests
