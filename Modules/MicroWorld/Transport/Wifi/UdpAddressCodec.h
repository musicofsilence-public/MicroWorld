...
/** Active byte count of a 6-byte UDP `FDeviceAddress`: four IPv4 octets plus two port bytes. */
inline constexpr std::uint8_t UdpAddressByteCount = 6;

/** Index of the first IPv4 octet stored in a UDP `FDeviceAddress`. */
inline constexpr std::uint8_t UdpAddressOctetAIndex = 0;

/** Index of the second IPv4 octet stored in a UDP `FDeviceAddress`. */
inline constexpr std::uint8_t UdpAddressOctetBIndex = 1;

/** Index of the third IPv4 octet stored in a UDP `FDeviceAddress`. */
inline constexpr std::uint8_t UdpAddressOctetCIndex = 2;

/** Index of the fourth IPv4 octet stored in a UDP `FDeviceAddress`. */
inline constexpr std::uint8_t UdpAddressOctetDIndex = 3;

/** Index of the high (most-significant) byte of the network-order port in a UDP `FDeviceAddress`. */
inline constexpr std::uint8_t UdpAddressPortHighByteIndex = 4;

/** Index of the low (least-significant) byte of the network-order port in a UDP `FDeviceAddress`. */
...
/**
 * Encodes an IPv4 UDP endpoint into an opaque 6-byte `FDeviceAddress`.
 *
 ...
 */
constexpr FDeviceAddress MakeUdpAddress(
 ...
 */
constexpr bool IsUdpAddress(const FDeviceAddress& InAddress) noexcept
{
 ...
 */
constexpr std::uint16_t UdpAddressPort(const FDeviceAddress& InAddress) noexcept
{
 ...
 * @return Six-byte address carrying the octets and the big-endian port.
 */
constexpr FDeviceAddress MakeUdpAddressFromPackedHostOrder(const std::uint32_t InPackedIpv4Address, const std::uint16_t InPort) noexcept
{
