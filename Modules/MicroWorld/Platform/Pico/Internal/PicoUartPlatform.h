...
/**
 * Narrow Pico UART binding used to test byte-stream policy without Pico SDK headers.
 *
 * Byte streams borrow this interface for their lifetime. It is a PlatformPico implementation detail rather than a
 * reusable hardware abstraction or supported direct-composition API.
 */
class IPicoUartPlatform
{
public:
	/** Releases a UART opened by this platform binding. */
	virtual ~IPicoUartPlatform() noexcept = default;

	/**
	 * Opens and configures one Pico UART for requested byte-stream settings.
	 *
...
protected:
	/** Restricts construction to concrete bindings because callers depend only on the stable interface. */
